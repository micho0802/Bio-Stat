...
ld_change_scores |> 
  summarize(
    ...
    change_sd = sd(change))
```
The average colony count per cubic foot change from before and after remediation, 1.6, so a moderate improving. The standard deviation, 1.2, represents a moderate amount of variation in this variable.
...
ld_change_scores |>
  summarize(
    before_median=quantile(change, probs=0.5),
    before_min=quantile(change, probs=0), 
    before_max=quantile(change, probs=1)) 
```
The median is close to the mean, indicating a fairly balanced distribution of changes, but with some rooms showing larger or smaller effects of remediation. The range is moderate, suggesting that while the remediation generally worked, its effectiveness varied across different rooms.